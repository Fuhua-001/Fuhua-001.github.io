...
app.get('/api/dashboard-stats', async (req, res) => {
  try {
    ...
    const quotesResRaw = await db.query('SELECT COUNT(*) as total_quotes FROM sales_pr');
    const customersResRaw = await db.query('SELECT COUNT(*) as total_customers FROM customers');
    const productsResRaw = await db.query('SELECT COUNT(*) as total_products FROM products');
    const monthlyResRaw = await db.query(`
    ...
    const salesRows = Array.isArray(salesResRaw[0]) ? salesResRaw[0] : (salesResRaw.rows || salesResRaw || []);
    const quotesRows = Array.isArray(quotesResRaw[0]) ? quotesResRaw[0] : (quotesResRaw.rows || quotesResRaw || []);
    const customersRows = Array.isArray(customersResRaw[0]) ? customersResRaw[0] : (customersResRaw.rows || customersResRaw || []);
    const productsRows = Array.isArray(productsResRaw[0]) ? productsResRaw[0] : (productsResRaw.rows || productsResRaw || []);
    const monthlyRows = Array.isArray(monthlyResRaw[0]) ? monthlyResRaw[0] : (monthlyResRaw.rows || monthlyResRaw || []);
...
app.post("/api/employees", async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO employees (pic_code, pic_name, pic_name_eng, contact_number, department, keywords) VALUES (?, ?, ?, ?, ?, ?)', [pic_code, pic_name, pic_name_eng || null, contact_number, department || null, keywords]);
