...
exports.saveQuote = async (req, res) => {
  try {
    const { customer_name, total_amount, items, ai_prompt, credit_days, payment_terms, pic_code } = req.body;
    ...
    let c_code = null, c_email = null, c_phone = null, c_pic = pic_code || "EMP-001", c_contact = customer_name;
      ...
      const itemQueries = items.map(async (item) => {
        ...
        await conn.query(
          `INSERT INTO sub_sales_pr 
                  ...
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            salesPrId, p_code, item.description, ai_prompt || "",
            item.quantity, p_unit, item.unit_price, rowTotal,
            rowTotal * 0.07, rowTotal * 1.07,
          ],
        );
      });
